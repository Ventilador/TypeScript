...
import { IRequestContext, Dictionary } from "../../definitions";
const ts = ts_ as any;
// const commentExp = /^\/\/\s?thingy-dive/;
// const commentExp2 = /^\/\*\s?thingy-dive\s?\*\//;

export function CollectExternalModuleReferences() {
    return function CollectExternalModuleReferences(this: Waterfall<IRequestContext>, request: IRequestContext) {
        const options = this.options;
        const deepDive = false; //  commentExp.test(request.data) || commentExp2.test(request.data);
        const file = request.sourceFile as any;
        let imports: any[] | undefined = undefined;
        let moduleAugmentations: any;
        let ambientModules: any[] | undefined = undefined;
        if (!file.imports) {
        ...
        function collectModuleReferences(node: any, inAmbientModule: any) {
            switch (node.kind) {
                ...
                case ts_.SyntaxKind.ExportDeclaration:
                    ...
                    if (!inAmbientModule || !ts_.isExternalModuleNameRelative(moduleNameExpr.text)) {
...
function pushTo(this: any[], item: any) {
    this.push(item);
}
