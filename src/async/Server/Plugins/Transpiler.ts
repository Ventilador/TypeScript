...
import { SourceFile } from '../../../../lib/typescript';
const {
...
class CompilerHost {
    ...
    private inputFileName: string | undefined;
    ...
    flush(err?: any) {
        const toReturn = err ?
            { outputText: undefined, diagnostics: undefined, sourceMapText: undefined } :
            { outputText: this.outputText, diagnostics: undefined, sourceMapText: this.sourceMapText };
        this.sourceMapText = this.outputText = this.inputFileName = this.sourceFile = undefined;
        return toReturn;
    }
    getSourceFile() {
...
export function transpile(input: SourceFile, transpileOptions: ts.TranspileOptions) {
    ...
    return _transpile(program, compilerHost);
}

function _transpile(program: ts.Program, compilerHost: CompilerHost) {
    let threw = null;
    try {
